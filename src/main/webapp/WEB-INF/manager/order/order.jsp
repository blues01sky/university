...
<body>
	...
	<div class="nav col-lg-11">
		<table class="table table-striped">
			...
			<%
				List<Order> orders = (List<Order>)request.getAttribute("lists");
				if (orders.isEmpty()) {
					
			%>
			<script>
				
				if (confirm("订单为空，是否返回管理员页？")) {
			...
			<tr>
				...
				<td><a
					href="javascript:if(confirm('是否删除订单号为<%=order.getOrderid() %>的订单？')) location.href='admindeleteorder.order?orderid=<%=order.getOrderid() %>'">删除</a>
				</td>
				<td><a
					href="javascript:if(confirm('是否彻底删除订单号为<%=order.getOrderid() %>的订单？')) location.href='sureadmindeleteorder.order?orderid=<%=order.getOrderid() %>'">彻底删除</a>
				</td>
			</tr>
			<% 
						}
