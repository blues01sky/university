...
<script type="text/javascript">
		...
		System.out.println(msg+"1234");
		if (msg == "恭喜登录成功") {%>
			window.onload = function(){
				alert("恭喜登录成功")}
			<%}else if (msg == "密码错误请再次尝试") {%>
			window.onload = function(){
				alert("密码错误请再次尝试")}
			<%}else if (msg=="用户不存在") {%>
			window.onload = function(){
				alert("用户不存在")}
			<%}else if (msg == "恭喜重置密码成功") {%>
			window.onload = function(){
				alert("恭喜重置密码成功")}
			<%}else if (msg == "恭喜修改用户信息成功") {%>
				window.onload = function(){
				alert("恭喜修改用户信息成功")}
			<%}else if (msg == "用户已存在") {%>
				window.onload = function(){
				alert("用户已存在，请更改用户名重新尝试")}
			<%}else if (msg == "恭喜注册成功") {%>
				window.onload = function(){
				alert("注册一个新用户成功，请登录")}
		<%}%>
						...
						<div class="form">
							...
							<form action="<%=basePath%>user/regist" method="post">
								<input type="text" name="Username" placeholder="用户名"
									...
									name="score" placeholder="分数" required="required" /> <input
									type="text" name="province" placeholder="地区"
									required="required" /> <input type="text"
									contenteditable="false" name="department" list="department1"
									placeholder="选科（默认为理科）" required="required" />
