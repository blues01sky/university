	...
	<div class="nav col-lg-1">
		...
		<div class="nav col-lg-11" style="padding-bottom: 30px;">
			<table class="table table-striped">
				...
				<%
					List<Charge> result = (List<Charge>)request.getAttribute("result");
					for(Charge charge : result){
						%>
				<tr>
					<td><%=charge.getId() %></td>
					<td><%=charge.getUniversityname() %></td>
					<td><%=charge.getProvince() %></td>
					<td><%=charge.getType() %></td>
					<td title="<%=charge.getCost() %>">鼠标悬停点击查看详情</td>
					<td>
						<a href="<%=basePath%>charge/addcharge">增加</a>
					</td>
					<td>
						<a href="<%=basePath%>charge/updatecharge?chargeid=<%=charge.getId() %>">修改</a>
					</td>
					<td>
						<a href="javascript:if(confirm('确实要删除<%=charge.getUniversityname() %>吗？')) location.href='<%=basePath%>charge/delcharge?chargeid=<%=charge.getId() %>'">删除</a>
					</td>
				</tr>
				<%
					}
